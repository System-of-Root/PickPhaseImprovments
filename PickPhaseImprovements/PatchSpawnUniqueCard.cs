...
namespace PickPhaseImprovements{
    ...
    public class PatchSpawnUniqueCard{
        public static bool Prefix(CardChoice __instance, Vector3 pos, Quaternion rot, ref GameObject __result){
            ...
            string PrefabToSpawn = PickManager.AlternetSpawnName.ContainsKey(CardToSpawn)?  PickManager.AlternetSpawnName[CardToSpawn] : CardToSpawn.name;
            if (PickManager.Synchronous){
                __result = PhotonNetwork.prefabPool.Instantiate(PrefabToSpawn, pos, rot);
                ...
                __result = PhotonNetwork.Instantiate(PrefabToSpawn, pos, rot,data:PickManager.CustomPhotonData.GetValueOrDefault(CardToSpawn));
